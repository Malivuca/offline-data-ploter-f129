...
def plotar_tabela(intervalos, casas_decimais, valor_medio, desvio_padrao, incerteza_padrao, local):
	...
	column_labels = ["<b>INTERVALOS</b>", "<b>OCORRÊNCIAS</b>", "<b>FREQUÊNCIA RELATIVA</b>"]
	...
	for i in range(1, 1001):
		if os.path.exists("images/tabela_de_ocorrencias.jpg"):
			if not os.path.exists("images/tabela_de_ocorrencias(%d).jpg" % i):
				fig.write_image("images/tabela_de_ocorrencias(%d).jpg" % i, format="jpeg", scale=2)
				break
		else:
			fig.write_image("images/tabela_de_ocorrencias.jpg", format="jpeg", scale=2)
			break
